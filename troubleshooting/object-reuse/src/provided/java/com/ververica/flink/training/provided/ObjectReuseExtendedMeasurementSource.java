...
public class ObjectReuseExtendedMeasurementSource extends
	...
	private transient ExtendedMeasurement.Sensor[] sensors;
	private transient ExtendedMeasurement.Location[] locations;
	...
	public void open(final Configuration parameters) {
		...
		getRuntimeContext().getMetricGroup().gauge(
				"sourceTemperatureUSmin",
				new MeanGauge.MinGauge(sourceTemperatureUS));
		getRuntimeContext().getMetricGroup().gauge(
				"sourceTemperatureUSmax",
				new MeanGauge.MaxGauge(sourceTemperatureUS));
	}
	...
	private void initSensors() {
		final SplittableRandom rnd = new SplittableRandom();
		final ExtendedMeasurement.SensorType[] sensorTypes = ExtendedMeasurement.SensorType.values();

		final int start = getRuntimeContext().getIndexOfThisSubtask() * NUM_SENSORS;
		this.sensors = new ExtendedMeasurement.Sensor[NUM_SENSORS];
		this.lastValue = new double[NUM_SENSORS];
		this.locations = new ExtendedMeasurement.Location[NUM_SENSORS];
		for (int i = 0; i < NUM_SENSORS; ++i) {
			long sensorId = start + i;
			long vendorId = sensorId % 100;
			final ExtendedMeasurement.SensorType sensorType =
					sensorTypes[(i / 2) % sensorTypes.length];
			sensors[i] = new ExtendedMeasurement.Sensor(sensorId, vendorId, sensorType);
	...
	private double randomInitialMeasurementValue(
			...
			ExtendedMeasurement.SensorType sensorType) {
		switch (sensorType) {
			case Temperature:
				// -10°C - 35°C
				return rnd.nextInt(
	 ...
	 */
	private static ExtendedMeasurement.Location randomInitialLocation(
			SplittableRandom rnd, int i) {
		...
		return new ExtendedMeasurement.Location(longitude, latitude, height);
	}
	...
	private ExtendedMeasurement randomEvent(SplittableRandom rnd) {
		...
		ExtendedMeasurement.Sensor sensor = sensors[randomIdx];
		ExtendedMeasurement.Location location = locations[randomIdx];
		...
		final ExtendedMeasurement.MeasurementValue measurement =
		...
		return new ExtendedMeasurement(
				new ExtendedMeasurement.Sensor(
						sensor.getSensorId(), sensor.getVendorId(), sensor.getSensorType()),
				new ExtendedMeasurement.Location(
	...
	private double randomChangeMeasurementValue(
			SplittableRandom rnd,
			ExtendedMeasurement.SensorType sensorType,
			ExtendedMeasurement.Location location,
			double lastValue) {
