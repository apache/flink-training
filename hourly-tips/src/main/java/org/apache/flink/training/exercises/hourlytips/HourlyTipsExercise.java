...
public class HourlyTipsExercise {
    ...
    public static class AddDriverTips extends ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow> {
        @Override
        public void process(Long key,
                            Context context,
                            Iterable<TaxiFare> taxiFareElements,
                            Collector<Tuple3<Long, Long, Float>> out) {
            Float sumOfDriverTips = 0F;
            for (TaxiFare taxtFare : taxiFareElements) {
                sumOfDriverTips += taxtFare.tip;
            }
            out.collect(Tuple3.of(context.window().getEnd(), key, sumOfDriverTips));
        }
